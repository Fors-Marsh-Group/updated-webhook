...
func Dumper(w io.Writer) func(http.Handler) http.Handler {
	return func(h http.Handler) http.Handler {
		return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
			...
			// Request parameters
			keys = make([]string, len(chi.RouteContext(r.Context()).URLParams.Keys))
			for i := range chi.RouteContext(r.Context()).URLParams.Keys {
				keys[i] = chi.RouteContext(r.Context()).URLParams.Values[i]
			}
			sort.Strings(keys)
			for _, k := range keys {
				buf.WriteString(fmt.Sprintf("\n> [%s] %s: %s", rid, k, strings.Join(r.Header[k], ", ")))
